...
var AI_DIFFICULTY = {
  ...
  // NOT hand-tuned. Bred by the genetic algorithm in train/ over thousands of
  // simulated matches, selected to win a field of cheating HUNTERS (it beat that
  // field ~72% of the time, vs ~20% for a cheating hunter itself). The strategy
  // it discovered: a fast, evasive PACIFIST — sprints constantly, hoards open
  // space (positive `open`), looks very far ahead (deep `flood`), bails out of a
  // tightening pocket extremely early (huge `escapeSpace`), dodges incoming
  // rockets, and never attacks — letting reckless breaching rivals destroy each
  // other. Use with the BALANCED character to get the profile exactly as evolved.
  evolved: {
    aimRange: 2042,
    aimTake: 1,
    lead: true,
...
var Game = class {
  ...
  trailGrads = [];
  constructor(canvas, cols, rows, cell, viewW, viewH, cb) {
    this.cols = cols;
    this.rows = rows;
    this.cell = cell;
    this.viewW = viewW;
  ...
  // ---- match setup -------------------------------------------------------
  newMatch(config) {
    const base2 = SPEED_INTERVAL[config.speed];
    ...
    configs.forEach((c, i) => {
      const p = new Player(i, c.name, c.color, c.type, base2, c.keys);
      if (c.type === "ai") {
  ...
  buildQuadArena(spec) {
    ...
    for (let q = 0; q < quads.length; q++) {
      ...
      const n = spec.perQuadrant;
      const span = Math.min(qw, qh) / 2;
      ...
      for (let r = 0; r < rings; r++) {
        radii.push(
          rings === 1 ? minR : minR + (maxR - minR) * r / (rings - 1)
        );
      }
  ...
  update(dt) {
    if (this.state === "playing") {
      ...
      this.tickMs = performance.now() - t02;
    }
  ...
  resolveRoster(roster, n) {
    ...
    if (roster.mode === "random") {
      const pool2 = roster.pool.length ? roster.pool : ALL_PERSONALITIES;
      for (let i = 0; i < n; i++) {
        out.push(pool2[Math.floor(Math.random() * pool2.length)]);
      }
      return this.shuffle(out);
    }
    const entries = Object.entries(roster.counts).filter(([, c]) => c > 0);
  ...
  aiChoose(p) {
    ...
    for (let d = 0; d < 4; d = d + 1) {
      ...
      let score = this.floodCount(nx, ny, cfg.flood);
      if (pathDir !== null && d === pathDir && score >= PF_MIN_FLOOD) {
        score += PF_PATH_BONUS;
      }
      score += this.openness(nx, ny, OPEN_RADIUS) * cfg.open;
      if (target) score += this.aimBonus(p, d, target) * cfg.hunt;
  ...
  nearestRival(p, range) {
    ...
    for (let r = 0; r <= maxR; r++) {
      ...
      if (best2 && foundRing < 0) foundRing = r;
    }
    return best2;
  }
  ...
  //   2. otherwise, don't CONVERGE: never drive into a cell a nearby rival is also
  //      about to enter (a head-on / closing crash), and don't hug a living rival
  //      (the instant it dies it stamps a 3x3 of solid death cells that box us in).
  //      In a dense field two cycles steering onto the same cell is by far the
  //      biggest killer, so we read each nearby rival's NEXT step and refuse to
  //      pile into it — swinging to a clear, non-converging side instead.
  ...
  aiAvoidDanger(p) {
    ...
    const nx = p.x + DELTA[p.dir].x;
    const ny = p.y + DELTA[p.dir].y;
    if (!this.cellConverges(p, nx, ny)) return;
    for (const d of [(p.dir + 1) % 4, (p.dir + 3) % 4]) {
      if (d === back) continue;
      const ax = p.x + DELTA[d].x;
      const ay = p.y + DELTA[d].y;
      if (this.isFree(ax, ay) && !this.cellConverges(p, ax, ay)) {
        p.dir = d;
        return;
      }
    }
  }
  // Would entering (x,y) risk a crash with a nearby rival? True if a living rival's
  // head sits on (x,y) or is about to step onto it next move (a converging /
  // head-on collision). Scans only the spatial-grid tiles around `p`, so it stays
  // cheap even with hundreds of cycles. This is the hunter's anti-pileup reflex:
  // in a dense field two cycles steering onto the same cell is the #1 killer, and
  // static survival steering can't see it because the cell is still empty NOW.
  cellConverges(p, x, y) {
    const tx = this.tilesX;
    const ty = this.tilesY;
    const start = this.tileStart;
    ...
    for (let dy = -1; dy <= 1; dy++) {
      for (let dx = -1; dx <= 1; dx++) {
        ...
        for (let k = start[t]; k < e; k++) {
          ...
          const qnx = this.wrapX(q.x + DELTA[q.dir].x);
          const qny = this.wrapY(q.y + DELTA[q.dir].y);
          if (qnx === x && qny === y) return true;
        }
      }
    }
    return false;
  ...
  aiMaybeShoot(p) {
    ...
    if (target) {
      ...
      if (cfg.dodge && target.aiProfile?.dodge && Math.random() < HUNTER_TRUCE) {
        return;
      }
    ...
    if (cfg.breach && this.selfBlastSafe(p)) {
      ...
      const trucePrey = cfg.dodge && prey?.aiProfile?.dodge && Math.random() < HUNTER_TRUCE;
      if (prey && !trucePrey && this.headingToward(p, prey) && this.obstacleAhead(p, 8)) {
  ...
  openSpace(p, cap) {
    ...
    for (let d = 0; d < 4; d = d + 1) {
      ...
      const ny = p.y + DELTA[d].y;
      if (!this.isFree(nx, ny)) continue;
      best2 = Math.max(best2, this.floodCount(nx, ny, cap));
      if (best2 >= cap) break;
    }
    return best2;
  }
  ...
  drawTrail(x, y, id, base2) {
    ...
    const px = x * cell;
    const py = y * cell;
    const dir = dirs[base2];
    const gx = base2 % cols;
    const gy = (base2 - gx) / cols;
    let fwd = -1;
    for (let d = 0; d < 4; d = d + 1) {
      let nx = gx + DELTA[d].x;
      let ny = gy + DELTA[d].y;
...
var MatchPool = class {
  ...
  constructor(n) {
    ...
    this.workers = Array.from({ length: this.size }, () => new Worker2(WORKER_FILE));
  }
...
var GENS = Number(process.env.GENS ?? 8);
var MATCHES = Number(process.env.MATCHES ?? 16);
var ELITE = Number(process.env.ELITE ?? 4);
var FIELD = Number(process.env.FIELD ?? 6);
var STEP_CAP = Number(process.env.STEP_CAP ?? 3e3);
var GA_SEED = Number(process.env.SEED ?? 1234);
var WORKERS = Number(process.env.WORKERS ?? cpus().length);
var DIFFICULTY = process.env.DIFFICULTY ?? "cheating";
var PERSONA = process.env.PERSONA ?? "hunter";
var CONFIG = {
  humans: 0,
  ai: FIELD,
  speed: "normal",
  difficulty: DIFFICULTY,
  // reference opponents the candidate must beat
  map: "cross",
...
var RUNGS = parseRungs(process.env.RUNGS) ?? defaultRungs();
...
function randomizeFrom(base2) {
  const g = structuredClone(base2);
  g.lead = true;
  g.escape = true;
  g.seekRange = base2.seekRange ?? base2.aimRange;
  for (const gene of NUM_GENES) {
    const cur = g[gene.key] ?? 0;
    let v = cur + gauss() * gene.sigma;
    v = clamp(v, gene.min, gene.max);
    g[gene.key] = gene.int ? Math.round(v) : v;
  }
  for (const b of BOOL_GENES) {
    const bv = base2[b] ?? false;
    g[b] = rng() < 0.3 ? !bv : bv;
  }
  return g;
}
function mutate(g) {
  ...
  for (const gene of NUM_GENES) {
    if (rng() < 0.5) {
      ...
      v = clamp(v, gene.min, gene.max);
      c[gene.key] = gene.int ? Math.round(v) : v;
    }
  }
...
function makeEvals(base2, count) {
  const out = [];
  for (let i = 0; i < count; i++) out.push({ seed: base2 + i * 7919 + 1, slot: i % FIELD });
  return out;
}
...
var matchesRun = 0;
async function evalSet(pool2, genomes, evals) {
  const jobs = [];
...
async function runGeneration(pool2, pop, base2, genBase) {
  const all = pop.map((g) => ({ g, score: 0, wins: 0, played: 0 }));
  ...
  for (const rung of RUNGS) {
    ...
    lastEvals = evals;
    const res = await evalSet(pool2, survivors.map((s) => s.g), evals);
  ...
  const refRes = (await evalSet(pool2, [base2], refTotal))[0];
  return {
    all,
    survivors,
...
var base = baselineProfile(CONFIG);
console.log("baseline (reference) profile:", JSON.stringify(base), "\n");
var pool = new MatchPool(WORKERS);
var population = [structuredClone(base)];
while (population.length < POP) population.push(randomizeFrom(base));
var logRows = ["gen,bestAvg,bestWins,meanAvg,refAvg,refWins,matchesRun"];
var best = null;
var t0 = Date.now();
for (let gen = 0; gen < GENS; gen++) {
  const { all, survivors, refAvg, refWins } = await runGeneration(pool, population, base, gen * 1000003);
  ...
  console.log(
    `gen ${String(gen).padStart(2)}  best=${avgOf(top).toFixed(3)} (wins ${top.wins}/${top.played})  mean=${mean.toFixed(3)}  ref=${refAvg.toFixed(3)} (wins ${refWins}/${MATCHES})  [${matchesRun} matches, ${((Date.now() - t0) / 1e3).toFixed(0)}s]`
  );
  logRows.push(
...
var valEvals = [];
for (let i = 0; i < 60; i++) valEvals.push({ seed: 7e6 + i * 104729 + 1, slot: i % FIELD });
var [evolvedVal, refVal] = await evalSet(pool, [best.g, base], valEvals);
