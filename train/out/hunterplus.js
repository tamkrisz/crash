...
var POP = Number(process.env.POP ?? 32);
var GENS = Number(process.env.GENS ?? 30);
var K = Number(process.env.K ?? 8);
var ELITE = Number(process.env.ELITE ?? 5);
var STEP_CAP = Number(process.env.STEP_CAP ?? 4e3);
var GA_SEED = Number(process.env.SEED ?? 7777);
var WORKERS = Number(process.env.WORKERS ?? cpus().length);
var PREY_DIFF = process.env.PREY_DIFF ?? "hard";
var SLOTS = 128;
var CONFIG = {
  humans: 0,
  ai: SLOTS,
...
var BOOL_GENES = ["alwaysSprint", "stalk", "breach", "pathfind", "dodge"];
...
function randomizeFrom(base) {
  ...
  for (const gene of NUM_GENES) {
    ...
    g[gene.key] = gene.int ? Math.round(clamp(v, gene.min, gene.max)) : clamp(v, gene.min, gene.max);
  }
...
function mutate(g) {
  ...
  for (const gene of NUM_GENES) if (rng() < 0.5) {
    const v = c[gene.key] + gauss() * gene.sigma * 0.6;
...
var PREY_PERSONAS = ["balanced", "packer", "runner", "roamer", "ambusher"];
function preyProfile(p) {
  return baselineProfile({
    ...CONFIG,
    difficulty: PREY_DIFF,
    mode: "classic",
    size: "small",
    roster: { mode: "uniform", personality: p, counts: {}, pool: [] }
  });
}
var PREY = PREY_PERSONAS.map(preyProfile);
var matchesRun = 0;
async function evalPop(pool2, pop, genBase) {
  ...
  for (let k = 0; k < K; k++) {
    ...
    const order = shuffledSlots(SLOTS, seed ^ 2654435769);
...
var hunterSeed = baselineProfile({
  ...CONFIG,
  mode: "classic",
  size: "small",
...
var population = [structuredClone(hunterSeed)];
while (population.length < POP) population.push(randomizeFrom(hunterSeed));
var seedFit = (await evalPop(pool, [hunterSeed], 12345e3))[0];
console.log(`stock cheating-hunter baseline: ${seedFit.toFixed(2)} kills/match
`);
var logRows = ["gen,bestKills,meanKills,seedKills,matchesRun"];
var best = null;
var t0 = Date.now();
for (let gen = 0; gen < GENS; gen++) {
  const fits = await evalPop(pool, population, gen * 1000003);
  ...
  console.log(
    `gen ${String(gen).padStart(2)}  best=${fits[top].toFixed(2)} kills  mean=${mean.toFixed(2)}  stockHunter=${seedFit.toFixed(2)}  [${matchesRun} matches, ${((Date.now() - t0) / 1e3).toFixed(0)}s]`
  );
...
var finalFit = (await evalPop(pool, [best.g], 5555e3))[0];
var stockFinal = (await evalPop(pool, [hunterSeed], 5555e3))[0];
console.log(`
