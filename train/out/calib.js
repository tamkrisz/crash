...
// train/calib.ts
var SURVIVED = Number.MAX_SAFE_INTEGER;
var MATCHES = Number(process.env.MATCHES ?? 60);
var STEP_CAP = Number(process.env.STEP_CAP ?? 4e3);
function roster(personality) {
  return { mode: "uniform", personality, counts: {}, pool: [] };
}
function base(ai, personality) {
  return {
    humans: 0,
    ai,
    speed: "normal",
    ...
    size: "medium",
    mode: "classic",
    roster: roster(personality)
  };
}
var chars = ["balanced", "survivor", "runner", "hunter"];
...
for (const c of chars) {
  let both = 0, mutual = 0, self2 = 0, steps = 0;
  for (let seed = 1; seed <= MATCHES; seed++) {
    ...
    if (died === 0) both++;
      ...
      mutual += killed;
      self2 += Math.max(0, died - killed);
    }
    steps += r.steps;
  }
  console.log(`  ${c.padEnd(9)} both=${(100 * both / MATCHES).toFixed(0)}%  mutual=${(100 * mutual / MATCHES).toFixed(0)}%  self=${(100 * self2 / MATCHES).toFixed(0)}%  avgSteps=${(steps / MATCHES).toFixed(0)}`);
}
