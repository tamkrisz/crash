...
var Game = class {
  ...
  pathToward(p, t) {
    ...
    while (hlen > 0 && pops < PF_NODES) {
      ...
      for (let d = 0; d < 4; d = d + 1) {
        ...
        const extra2 = this.grid[nc] !== EMPTY && this.wallRunAhead(nx, ny, opposite(d)) > PF_MAX_WALL_RUN ? PF_WALL_RUN_PENALTY : 0;
        const nd = cost + w + extra2;
  ...
  aiMaybeShoot(p) {
    ...
    if (cfg.breach && this.selfBlastSafe(p)) {
      const prey2 = this.nearestRival(p, cfg.seekRange ?? cfg.aimRange);
      if (prey2 && this.headingToward(p, prey2) && this.obstacleAhead(p, 8)) {
        const within = this.manhattan(p, prey2) <= PF_RANGE;
        const pd = within ? this.pathToward(p, prey2) : null;
...
var K = Number(process.env.K ?? 60);
var COPIES = Number(process.env.COPIES ?? 16);
var STEP_CAP = Number(process.env.STEP_CAP ?? 4500);
...
var profFor = (difficulty, personality) => baselineProfile({
  ...CONFIG,
  difficulty,
  mode: "classic",
  size: "small",
...
var extra = (process.env.PROFILES ?? "train/out/hunterplus-best.json").split(",").filter(Boolean);
var contenders = [
  { label: "stock cheating-hunter", prof: profFor("cheating", "hunter") },
  ...extra.map((p) => ({ label: p.split("/").pop(), prof: JSON.parse(readFileSync(p, "utf8")) }))
];
var prey = ["balanced", "packer", "runner", "roamer", "ambusher"].map((p) => profFor(PREY_DIFF, p));
...
var C = contenders.length;
var need = C * COPIES;
if (need > SLOTS) throw new Error(`${C} contenders x ${COPIES} copies = ${need} > ${SLOTS} slots`);
var jobs = [];
var maps = [];
for (let k = 0; k < K; k++) {
  const order = shuffled(SLOTS, 11256099 ^ k * 2654435761);
  const profiles = {};
  const slotsByContender = contenders.map(() => []);
  let p = 0;
  for (let c = 0; c < C; c++) for (let j = 0; j < COPIES; j++) {
    const sl = order[p++];
    profiles[sl] = contenders[c].prof;
    slotsByContender[c].push(sl);
  }
  for (; p < SLOTS; p++) profiles[order[p]] = prey[p % prey.length];
  jobs.push({ config: CONFIG, seed: 6e6 + k * 104729 + 1, scoredSlot: order[0], profiles, stepCap: STEP_CAP });
  maps.push(slotsByContender);
}
var replies = await pool.run(jobs);
var sum = contenders.map(() => 0);
var samples = K * COPIES;
...
`);
var ranked = contenders.map((c, i) => ({ label: c.label, kills: sum[i] / samples })).sort((a, b) => b.kills - a.kills);
