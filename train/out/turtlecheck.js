...
var Game = class {
  ...
  markSx = new Float64Array(10);
  markSy = new Float64Array(10);
  markDist = new Float64Array(10);
  markColor = new Array(10);
  ...
  visibleShifts(coord, camPx, viewSize, span, out) {
  ...
  setupParallel() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const cfg2 = p.aiProfile;
      if (!cfg2) continue;
      world.profHunt[i] = cfg2.hunt;
      world.profSeek[i] = cfg2.seekRange ?? cfg2.aimRange;
      world.profFlood[i] = cfg2.flood;
      world.profOpen[i] = cfg2.open;
      world.profStraight[i] = cfg2.straight;
      world.profJitter[i] = cfg2.jitter;
      world.profStalk[i] = cfg2.stalk ? 1 : 0;
      world.profPathfind[i] = cfg2.pathfind ? 1 : 0;
      world.profAvoid[i] = cfg2.avoid ?? 0;
      world.profAvoidRange[i] = cfg2.avoid ? cfg2.avoidRange ?? cfg2.seekRange ?? cfg2.aimRange : 0;
      world.profNoCoast[i] = cfg2.noCoast ? 1 : 0;
    }
  }
  ...
  aiThink(p) {
    const nx = p.x + DELTA[p.dir].x;
    const ny = p.y + DELTA[p.dir].y;
    const canCoast = !p.aiProfile?.noCoast && this.players.length >= AI_STAGGER_MIN_CYCLES && p.aiCooldown > 0 && p.escapeSteps === 0 && this.clearAhead(p, COAST_RUNWAY) >= COAST_RUNWAY && this.openness(nx, ny, OPEN_RADIUS) >= COAST_OPEN_MIN;
    if (canCoast) {
      p.aiCooldown--;
    } else {
  ...
  aiChoose(p) {
    const cfg2 = p.aiProfile ?? AI_DIFFICULTY[this.aiDifficulty];
    const cur = p.dir;
    const back = opposite(cur);
    if (p.escapeSteps > 0 && this.clearAhead(p, 2) >= 2) {
      return cur;
    }
    const target = cfg2.hunt > 0 && (p.charged || cfg2.stalk) ? this.nearestRival(p, cfg2.seekRange ?? cfg2.aimRange) : null;
    const avoidTarget = cfg2.avoid ? this.nearestRival(p, cfg2.avoidRange ?? cfg2.seekRange ?? cfg2.aimRange) : null;
    const pathDir = cfg2.pathfind && target && this.manhattan(p, target) <= PF_RANGE ? this.pathToward(p, target) : null;
    let bestDir = cur;
    ...
    for (let d = 0; d < 4; d = d + 1) {
      ...
      const flood = this.floodCount(nx, ny, cfg2.flood);
      let score = flood;
      if (pathDir !== null && d === pathDir && flood >= PF_MIN_FLOOD) {
        score += PF_PATH_BONUS;
      }
      score += this.openness(nx, ny, OPEN_RADIUS) * cfg2.open;
      if (target && flood >= HUNT_MIN_FLOOD) {
        score += this.aimBonus(p, d, target) * cfg2.hunt;
      }
      if (avoidTarget && cfg2.avoid) {
        score -= this.aimBonus(p, d, avoidTarget) * cfg2.avoid;
      }
      if (d === cur) score += cfg2.straight;
      score += Math.random() * cfg2.jitter;
      if (score > bestScore) {
        bestScore = score;
        bestDir = d;
      }
  ...
  aiAvoidDanger(p) {
    const cfg2 = p.aiProfile ?? AI_DIFFICULTY[this.aiDifficulty];
    if (!cfg2.dodge || p.escapeSteps > 0) return;
  ...
  beyondWallSpace(p, d, cap) {
    ...
    if (wd === 0) return { space: 0, wallDist: 0 };
    const bx = p.x + dx * (wd + BLAST_RADIUS + 1);
    const by = p.y + dy * (wd + BLAST_RADIUS + 1);
    if (!this.isFree(bx, by)) return { space: 0, wallDist: wd };
  ...
  // The turtle's "calculate a route out" override (the escapePlan trait). Runs on
  ...
  aiEscapePlan(p) {
    const cfg2 = p.aiProfile ?? AI_DIFFICULTY[this.aiDifficulty];
    if (!cfg2.escapeAim || p.escapeSteps > 0) return;
    const cap = cfg2.escapeSpace;
    ...
    if (!p.charged) {
      ...
      const timeToFire = cellsToFire * p.baseInterval;
      const timeToCharge = (CHARGE_MAX - p.charge) / CHARGE_RATE;
      if (timeToCharge > timeToFire) return;
    }
    p.dir = exit.dir;
  }
  aiMaybeShoot(p) {
    ...
    const cfg2 = p.aiProfile ?? AI_DIFFICULTY[this.aiDifficulty];
    if (cfg2.pacifist) {
      if (cfg2.escape && this.selfBlastSafe(p) && this.obstacleAhead(p, 6) && this.openSpace(p, cfg2.escapeSpace) < cfg2.escapeSpace) {
        this.tryShoot(p);
        p.escapeSteps = ESCAPE_STEPS;
      }
      return;
    }
    const target = this.lineOfFireRival(p, cfg2.aimRange, cfg2.lead);
    if (target) {
      if (cfg2.dodge && this.shotWouldSelfKill(p, cfg2.aimRange)) return;
      if (Math.random() < cfg2.aimTake) this.tryShoot(p);
      return;
    }
    if (cfg2.breach && this.selfBlastSafe(p)) {
      const prey = this.nearestRival(p, cfg2.seekRange ?? cfg2.aimRange);
    ...
    if (Math.random() < cfg2.openRate && this.selfBlastSafe(p) && this.obstacleAhead(p, 4)) {
      this.tryShoot(p);
    }
  }
  ...
  nextAlive(current, delta) {
    ...
    if (!current || !current.alive) return first;
...
var game = new Game(STUB, 160, 110, 4, 640, 440, noopCb);
function runMatch(config, seed, cap) {
  seedRandom(seed);
  game.newMatch(config);
  const players = game.players;
  const n = players.length;
  const persona = players.map((p) => p.personality ?? "balanced");
  const diedAt = new Array(n).fill(Number.MAX_SAFE_INTEGER);
  const dt = players[0]?.baseInterval ?? 100;
  let steps = 0;
  while (game.state === "playing" && steps < cap) {
    game.update(dt);
    steps++;
    for (let i = 0; i < n; i++) {
...
{
  const FIELD = ["turtle", "survivor", "balanced", "runner", "hunter", "ambusher"];
  const PER = 2;
  const MATCHES = 400;
  const CAP = 4e3;
  const counts = Object.fromEntries(FIELD.map((p) => [p, PER]));
  const survSum = {};
  const survCnt = {};
  const wins = {};
...
{
  const CHARS = ["turtle", "survivor", "balanced", "runner"];
  const MATCHES = 300;
  const CAP = 6e3;
  console.log(`
...
{
  const FIELD = ["turtle", "survivor", "balanced", "hunter"];
  const PER = 8;
  const MATCHES = 120;
  const CAP = 4e3;
  const counts = Object.fromEntries(FIELD.map((p) => [p, PER]));
